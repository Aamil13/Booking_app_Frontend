...
const HeroFilter = (props: Props) => {
  ...
  const handleOption = (name: string, operation: string) => {
    setOption((prev) => {
      return {
        ...
        [name]:
          operation === "i"
            ? option[name as keyof typeof option] + 1
            : option[name as keyof typeof option] - 1,
      };
    });
  };
  ...
  const changePage = async () => {
    ...
    await router.push(
      `/searchresult/?destination=${destination.toLowerCase()}&startdate=${herodate[0].startDate}&enddate=${herodate[0].endDate}&adult=${option.adult}&children=${option.children}&room=${option.room}`,
    );
  };
      ...
      <div className="h-full flex max-md:flex-col  justify-between max-md:items-center  bg-white ">
        <section className="bg-white flex gap-2   items-center h-full p-2">
          ...
          <input
            ...
            className="outline-none py-2 bg-white max-lg:w-36 max-lg:text-sm"
            type="text"
        ...
        <section className="bg-white flex gap-2 items-center h-full p-2 relative">
          ...
          <span
            onClick={() => setShowHeroDate(!showHeroDate)}
            className="bg-white py-2"
          >{`${format(herodate[0].startDate, "dd/MM/yyyy")} to ${format(herodate[0].endDate, "dd/MM/yyyy")}`}</span>
          <AnimatePresence>
            {showHeroDate && (
              <motion.div
                className="shadow-lg absolute top-20 -left-10 max-md:z-10 max-md:top-10"
                ref={HeroRef}
        ...
        <section className="bg-white flex gap-2 items-center h-full p-2 relative">
          ...
          <span
            ...
            className="bg-white py-2 tabular-nums"
          >{`${option.adult} adult · ${option.children} children · ${option.room} room`}</span>

          <AnimatePresence>
            {openOption && (
              <motion.div
                className="absolute w-52 bg-white top-20 left-12 max-md:left-5 max-md:top-10 max-lg:-left-4 shadow-lg border-2"
                ref={OptionRef}
                initial={{ opacity: 0, y: "-10%" }}
                animate={{ opacity: 1, y: "5%" }}
                exit={{
                ...
                <div className="flex justify-between p-2 items-center">
                  ...
                  <div className=" w-1/2 flex justify-between items-center gap-2 ">
                    <button
                      ...
                      className="w-6 text-xl border-2 border-green-300 rounded-md"
                    >
                      +
                    </button>
        ...
        <section
          onClick={() => changePage()}
          className="bg-white flex gap-2 items-center h-full p-2"
        >
