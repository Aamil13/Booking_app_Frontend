...
const PaginationCom = () => {
  ...
  const handleSearch = async (e: any) => {
    ...
    if (type != null) {
      await Router.push(
        `/searchresult/?type=${type}&startdate=${startdate}&enddate=${enddate}&adult=${adult}&children=${children}&room=${room}&page=${e.target.innerText}`,
      );
      await dispatch(
        searchTypeResult({
          type: type,
      ...
      await Router.push(
        `/searchresult/?destination=${destination?.toLowerCase()}&startdate=${startdate}&enddate=${enddate}&adult=${adult}&children=${children}&room=${room}&page=${e.target.innerText}`,
      );
      await dispatch(
        searchResult({
  ...
  const handlePrevNext = async (Number: number) => {
    if (type != null) {
      await Router.push(
        `/searchresult/?type=${type}&startdate=${startdate}&enddate=${enddate}&adult=${adult}&children=${children}&room=${room}&page=${Number}`,
      );
      await dispatch(
        searchTypeResult({
          type: type,
      ...
      await Router.push(
        `/searchresult/?destination=${destination?.toLowerCase()}&startdate=${startdate}&enddate=${enddate}&adult=${adult}&children=${children}&room=${room}&page=${Number}`,
      );
      await dispatch(
        searchResult({
          destination: destination,
    ...
    <>
      {paginated.length <= 1 ? (
        ...
        <div className="max-w-full mx-auto mt-12  text-gray-600 md:px-8">
          ...
          >
            <button
              disabled={Number(paginated[0]) == Number(currentPage)}
              onClick={() => handlePrevNext(Number(currentPage) - 1)}
            ...
            <button
              disabled={paginated.length == Number(currentPage)}
              onClick={() => handlePrevNext(Number(currentPage) + 1)}
              className="hover:text-indigo-600 flex items-center gap-x-2"
            >
              Next
