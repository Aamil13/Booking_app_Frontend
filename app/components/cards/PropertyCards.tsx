...
const PropertyCards = ({ imglink, id }: datatype) => {
  ...
  const handleClick = () => {
    ...
    router.push(
      `/searchresult/?type=${hoteltype[id].type}&startdate=${new Date()}&enddate=${new Date(Date.now() + 60 * 60 * 24 * 1000)}&room=1&page=1`,
    );
  };
      ...
      {hoteltype?.[id] ? (
        <>
          ...
          <p className="text-xs font-medium">
            {hoteltype?.[id]?.count}{" "}
            <span className="lowercase">{hoteltype?.[id]?.type}</span>
          </p>
        </>
      ) : err ? (
