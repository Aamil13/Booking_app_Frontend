...
const ReserveModal = ({hotelid,setShowReserve,StartDate,EndDate}: Props) => {
  ...
  return (
    <div className='fixed top-1/3 bg-white w-1/2 max-sm:w-4/5 p-4 shadow-lg rounded-lg border-2 border-green-400'>
       ...
       <p onClick={()=>handleshow()} className='absolute top-0 right-0 max-sm:-right-4'><IoIosCloseCircle size={24}/></p>
       </div>
       </div>
       ...
       roomLoading ? <div className='min-h-[250px] flex justify-center items-center'><MoonLoader color="#36d7b7" /></div>
       :
        roomData ?
        roomData?.map((item)=>(
        ...
        <div className='flex w-2/5 justify-center items-start gap-4'>
           {
            ...
            item?.roomNumbers?.map((number)=>(
                ...
                <label className={`${!isAvailable(number) ? "line-through" : ""}  `} >{number?.number}</label>
                <input type="checkbox" value={number?._id+"/"+number?.number} disabled={!isAvailable(number)} onChange={(e)=>handleroomSelect(e)} />
                </div>
            ))
           }
        </div>
    ...
    <p>Something went wrong!</p>
        ...
        {
            roomData && <div className='flex justify-center'><button disabled={ReserveLoading} onClick={handleClick} className=' py-2 px-4 font-bold text-white rounded-xl shadow-lg bg-green-400'>{ReserveLoading ? <BeatLoader size={6} color="white" /> : "Reserve" }</button></div>
        }
    </div>
  )
}
