...
export const getCityCount = createAsyncThunk(
    ...
    async(_,{rejectWithValue})=>{
            try {
                const res = await axios.get("/api/hotel/gethotelsccount?cities=delhi,mumbai,hyderabad,lucknow,kashmir,kerala")
                return res.data
            } catch (error) {
...
export const HomeGuest = createAsyncThunk(
    ...
    async(_,{rejectWithValue})=>{
        try {
            const res = await axios.get("/api/hotel/getallhotels?featured=true&limit=7&min=100&max=501")
            return res.data
        } catch (error) {
            return rejectWithValue(error)
        }
...
const HotelSlice = createSlice({
    ...
    extraReducers:(builder)=> {
        ...
        builder.addCase(getCityCount.fulfilled,(state,action:PayloadAction<any>)=>{
            state.cityloading = false
            state.citydata = action.payload
        }),
        builder.addCase(getCityCount.rejected,(state,action:PayloadAction<any>)=>{
            state.cityloading = false
        ...
        builder.addCase(getHotelType.fulfilled,(state,action:PayloadAction<any>)=>{
            state.HotelTypeLoading = false
            state.HotelTypedata = action.payload
        }),
        builder.addCase(getHotelType.rejected,(state,action:PayloadAction<any>)=>{
        ...
        builder.addCase(HomeGuest.fulfilled,(state,action:PayloadAction<any>)=>{
            state.guestTypeLoading = false
            state.guestTypeData = action.payload
        }),
        builder.addCase(HomeGuest.rejected,(state,action:PayloadAction<any>)=>{
            state.guestTypeLoading = false
        ...
        builder.addCase(searchResult.fulfilled,(state,action:PayloadAction<any>)=>{
